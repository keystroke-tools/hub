...
unsafe extern "Rust" fn __getrandom_v03_custom(
    ...
    let size = len as u32;
    let buf = match hubble::crypto::rand(crypto::RandSize::Custom(size)) {
        Ok(buf) => buf,
        Err(e) => {
            hubble::log::error(&format!("Failed to generate random bytes: {}", e));
            return Err(getrandom::Error::UNSUPPORTED);
        }
    };

    if buf.len() != size as usize {
        hubble::log::error(&format!(
            "Generated buffer size does not match requested size: {} != {}",
            buf.len(),
            size
        ));
        return Err(getrandom::Error::UNSUPPORTED);
    ...
    // Copy the generated bytes to the destination pointer
    let dest_slice = unsafe { std::slice::from_raw_parts_mut(dest, size as usize) };
